...
```
```{r include=FALSE}
ggsave("~/Dropbox/Data/Projects/regression-trees/random_forest/plots/gini.svg", width = 8, height = 4)
```
...
optimal_split <- function(X, classes, n_splits = 50){
  ...
  # best prediction for these data are mean of the classes
  classes_split <- split(classes, X <= optimal_split)
  preds <- list(split0 = mean(classes_split[[2]], na.rm = TRUE), 
                split1 = mean(classes_split[[1]], na.rm = TRUE))
...
```{r include=FALSE}
ggsave("~/Dropbox/Data/Projects/regression-trees/random_forest/plots/optimal_split.svg", width = 8, height = 3)
```
...
decision_tree_classifier <- function(X, Y, gini_threshold = 0.4, max_depth = 5, min_rows = 5, branch_id = '0', splits = NULL){
  ...
  if (is_too_deep | is_pure_enough | too_few_observations){
    ...
    # continue splitting
    split0 <- decision_tree_classifier(
      X = X_split[[1]], 
      Y = Y_split[[1]], 
...
```
```{r include=FALSE}
ggsave("~/Dropbox/Data/Projects/regression-trees/random_forest/plots/example_data.png", width = 8, height = 6)
```
...
```
```{r include=FALSE}
ggsave("~/Dropbox/Data/Projects/regression-trees/random_forest/plots/logistic_classes.png", width = 8, height = 6)
```
...
```
```{r include=FALSE}
ggsave("~/Dropbox/Data/Projects/regression-trees/random_forest/plots/tree_classes.png", width = 8, height = 6)
```
...
```
```{r include=FALSE}
ggsave("~/Dropbox/Data/Projects/regression-trees/random_forest/plots/tree_boundaries.png", width = 8, height = 6)
```
...
The predictions are done at the wrong level