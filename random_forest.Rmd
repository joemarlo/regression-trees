---
title: "Random forest bottom up -- a tutorial"
author: "Joe Marlo"
date: "4/23/2021"
output:
...
This is a step-by-step guide to build a random forest algorithm in "base" R from the bottom-up. We'll start with the Gini index then move to decision trees and then ensemble methods. R has many packages that include -- faster and better -- implementations of random forest. The goal of this guide is to walk through the fundamentals in an interpretable format. The final algorithm will be functional but it is not robust to bugs or programming edge cases. I.e. just use [`ranger`](https://arxiv.org/pdf/1508.04409.pdf) if you want a production algorithm.

## Process

Random forests are an ensemble model consisting of many decision trees. We must first create a decision tree classifier which themselves require an impurity metric and recursive algorithm to split the branches. The steps can broken down as:  
...
```{r include=FALSE}
source("~/Dropbox/Data/Projects/wsb-discourse/analyses/plots/ggplot_settings.R")
```

## Gini index

.... The output of `gini` is constrained to `[0, 0.5]`.
...
Explain why it needs to be weighted
...
optimal_split <- function(X, classes, n_splits = 50){
  ...
  gini_minimum <- min(gini_index, na.rm = TRUE)[1]
  optimal_split <- na.omit(splits[gini_index == gini_minimum])[1]
  
  # best prediction for these data the mean of the classes
  preds <- mean(classes, na.rm = TRUE)
...
```{r optimal_split_plot }
# plot the points and the optimal split
tibble(x = X, y = 1, col = classes) %>%
...
To create the decision trees, we need to apply the spliting algorithm until it reaches a certain stopping threshold. We won't know prior how many splits it is going to make -- the depth or the width. Therefore we cannot use a `while` loop because a split results in two new branches. A standard `while` loop can only account for a single, direct process. We need to use [recursion](). 

In recursive functions, the function itself is called within itself until some stopping criteria is met. It will run indefinitely without the stopping criteria. A simple example is the [quicksort]() algorithm which sorts a vector of numbers from smallest to greatest. 

Quicksort splits the vector into two vectors based on a pivot point. Points smaller than the pivot go to one vector, points larger to the other vector. The pivot point can be any point but is often the first or last. The function is called on itself to repeat the splitting until one or less numbers exist in a split. Then these sorted child-vectors are passed upward through the recursed functions. 

See the below example.

```{r quicksort}
quick_sort <- function(X){
  # stop if X is length 1 or less
...
decision_tree_classifier <- function(X, Y, gini_threshold = 0.4, max_depth = 5, min_rows = 5, branch_id = '0', splits = NULL){
  
  # calculate the first best split
  ...
  # return the current list of splits or continue splitting
  is_too_deep <- isTRUE(branch_depth >= max_depth)
  is_pure_enough <- isTRUE(first_split$gini <= gini_threshold)
  too_few_observations <- isTRUE(min(sapply(X_split, nrow)) < min_rows)
  if (is_too_deep | is_pure_enough | too_few_observations){
    return(splits)
  } else {
    
    # continue splitting
    split1 <- decision_tree_classifier(
      X = X_split[[1]], 
      Y = Y_split[[1]], 
    ...
    split2 <- decision_tree_classifier(
      X = X_split[[2]], 
      Y = Y_split[[2]], 
      gini_threshold = gini_threshold, 
      max_depth = max_depth, 
    ...
    all_splits <- distinct(bind_rows(split1, split2))
...
X <- .data[, -1]
Y <- .data[[1]]
decision_tree <- decision_tree_classifier(X, Y)
decision_tree
```
...
predict_data_point <- function(model_decision_tree, new_row, branch_id = '0'){
  ...
  # if next branch_id is not in the decision tree then return the current prediction
  if (!(branch_id %in% model_decision_tree$branch)){
    return(decision_point$pred) 
  } else {
...
X <- .data[, -1]
Y <- .data[[1]]
model_decision_tree <- decision_tree_classifier(X, Y, max_depth = 5, gini_threshold = 0.01, min_rows = 3)
...
```{r bag_it}
bag_it <- function(X, Y, n_trees = 100, max_depth = 5, gini_threshold = 0.2, min_rows = 5){
  ...
  preds <- parallel::mclapply(1:n_trees, function(i){
    ...
    preds <- tryCatch({
      ...
      model_decision_tree <- decision_tree_classifier(X_sampled, Y_sampled, max_depth = max_depth, gini_threshold = gini_threshold, min_rows = min_rows)
      preds <- predict_decision_tree(model_decision_tree, X)
    },
    error = function(e) NA
    )
...
```{r random_forest}
random_forest <- function(X, Y, n_trees = 100, max_depth = 5, gini_threshold = 0.2, min_rows = 5){
