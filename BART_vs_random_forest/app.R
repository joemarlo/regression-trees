...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            br(),
            actionButton(inputId = 'button_newSplitX1',
                         label = "Add new split on X1"),
            actionButton(inputId = 'button_newSplitX2',
                         label = "Add new split on X2"),
            actionButton(inputId = 'button_removeSplit',
                         label = "Remove last split"),
        ...
        mainPanel(
            width = 8,
            plotOutput(outputId = "plot_regressionTree",
                       click = "plot_regressionTree_clickX",
                       dblclick = "plot_regressionTree_clickY")
        )
    ),
...
server <- function(input, output) {
    ...
    # store the last plot clicks
    # TODO: need to save record and if X/Y to one vector
    observeEvent(input$plot_regressionTree_clickX, {
        store$plot_clicks <- input$plot_regressionTree_clickX$x
    })
    observeEvent(input$plot_regressionTree_clickY, {
        store$plot_clicks <- input$plot_regressionTree_clickY$y
    })
    
    # insert new split with buttons
    # observeEvent(input$button_newSplitX1, {
    #     btn <- input$button_newSplitX1
    #     id <- paste0('button_X1_', btn)
    #     insertUI(
    #         selector = '#placeholder',
    #         ui = tags$div(
    #             id = id,
    #             sliderInput(inputId = paste0('slider_X1', id),
    #                         label = 'X1',
    #                         min = -1,
    #                         max = 1,
    #                         step = 0.01,
    ...
    #         )
    #     )
    #     store$button_ids <- append(store$button_ids, id)
    # })
    # observeEvent(input$button_newSplitX2, {
    #     btn <- input$button_newSplitX2
    #     id <- paste0('button_X2_', btn)
    #     insertUI(
    #         selector = '#placeholder',
    #         ui = tags$div(
    #             id = id,
    #             sliderInput(inputId = paste0('slider_X2_', id),
    #                         label = 'X2',
    #                         min = -1,
    #                         max = 1,
    #                         step = 0.01,
    ...
    # remove last split
    # observeEvent(input$button_removeSplit, {
    #     button_ids <- store$button_ids
    #     removeUI(selector = paste0('#', button_ids[length(button_ids)]))
    #     store$button_ids <- button_ids[-length(button_ids)]
    # })
    ...
    output$plot_regressionTree <- renderPlot({
        ...
        p <- dat %>% 
            ...
            # geom_vline(xintercept = store$plot_clicks[1]) +
            geom_hline(yintercept = store$plot_clicks[1]) +
